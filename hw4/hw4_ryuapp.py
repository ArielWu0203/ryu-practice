...
ryuapp_instance_name = 'hw4_RyuApp'
url = '/lldp/{dpid}'

class hw4_RyuApp(app_manager.RyuApp):
    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]
    lldp_struct = {}
    _CONTEXTS = {'wsgi':WSGIApplication}

    def __init__(self,*args,**kwargs):
        super(hw4_RyuApp,self).__init__(*args,**kwargs)
        wsgi = kwargs['wsgi']
        wsgi.register(hw4_RyuAPI,{ryuapp_instance_name:self})
...
class hw4_RyuAPI(ControllerBase):
    def __init__(self,req,link,data,**config):
        super(hw4_RyuAPI,self).__init__(req,link,data,**config)
        self.ryuapp_instance = data[ryuapp_instance_name]

    @route ('hw4_lldp',url,methods=['GET'])
    def get_lldp_struct(self,req,**kwargs):
        ryuapp = self.ryuapp_instance
        dpid = int(kwargs['dpid'])

        if dpid not in ryuapp.lldp_struct:
            return Response(status=404,body='Not found\n')

        lldp_info = ryuapp.lldp_struct.get(dpid)
        body = json.dumps(lldp_info,indent = 4,sort_keys = True)+'\n'
